...
public class PlayingState extends GameState {
	...
	public void draw(Graphics2D g) {
		...
		g.drawString(Integer.toString(GameData.getCurrentScore()), 760, 35);
		if(timeLimit != -1) {
	...
	private void eventFinish() {
		...
		if(eventCount >= 60 && fadeOutTimer == -1) {
			...
			// finished level
			if((LevelData.getLevelIndex() + 1) % 3 == 0) {
				nextState = GameStateManager.LEVEL_SELECT_STATE;
				GameData.setFinalScore(LevelData.getLevelIndex() / 3);
				GameData.save();
			}
